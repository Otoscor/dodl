  ...
  return (
    <div className="flex items-center border border-border-subtle rounded-lg overflow-hidden">
      <button
        ...
        disabled={quantity <= 1}
        className={`${btnSize} flex items-center justify-center text-text-secondary hover:bg-surface-elevated disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer`}
      >
        −
      </button>
      <span className={`${numSize} text-center text-text-primary font-mono tabular-nums`}>
        {quantity}
      </span>
      <button
